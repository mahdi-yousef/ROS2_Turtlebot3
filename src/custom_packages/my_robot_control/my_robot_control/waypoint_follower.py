...
# AMCL publishes /amcl_pose as transient-local (latched) so a late subscriber
# still gets the last known pose. The default subscription QoS is volatile,
# which is incompatible with that -- if the 2D Pose Estimate was already
# given before this node starts, a volatile subscriber never receives the
# already-published message and waits forever. Matching durability here
# fixes that.
...
# How long to wait after the first /amcl_pose message before trusting it.
# That first message just reflects AMCL seeding its particle filter from the
# RViz click, not a converged estimate -- starting immediately can send the
# first goal against a still-settling localization.
LOCALIZATION_SETTLE_SEC = 3.0
...
class WaypointFollower(Node):
    ...
    def _on_amcl_pose(self, msg):
        ...
        self.get_logger().info(
            f'AMCL pose received -- waiting {LOCALIZATION_SETTLE_SEC:.0f}s for '
            'localization to settle before starting waypoint mission.'
        )
        self.destroy_subscription(self._amcl_sub)
        self._settle_timer = self.create_timer(
            LOCALIZATION_SETTLE_SEC, self._on_settle_timer)

    def _on_settle_timer(self):
        self._settle_timer.cancel()
        self.get_logger().info('Localization settled -- starting waypoint mission.')
        self._start()
